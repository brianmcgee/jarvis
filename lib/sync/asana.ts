...
async function upsertTask(db: PrismaClient, task: Task) {
    ...
    const {
        gid, name, notes, html_notes: notesHtml, liked, num_likes: numLikes, num_subtasks: numSubtasks,
        followers, parent, permalink_url: permalinkUrl, projects, created_at: createdAt,
        modified_at: modifiedAt, completed_at: completedAt, due_on, completed, workspace, assignee
    } = (task as Task)
    ...
    await db.task.upsert({
        where: {gid: task.gid},
        create: {
            gid, name, notes, notesHtml, liked, numLikes, numSubtasks, permalinkUrl,
            ...
            createdAt, modifiedAt, dueAt, completedAt, completed
        },
        update: {
            name, notes, notesHtml, liked, numLikes, numSubtasks, permalinkUrl,
            followers: connectOrCreateFollowers,
            projects: connectOrCreateProjects,
            parent: connectParent,
            workspace: {connect: connectWorkspace},
...
async function fullSync(db: PrismaClient = new PrismaClient()) {
    ...
    const opt_fields = [
        'id', 'name', 'notes', 'html_notes', 'liked', 'num_likes', 'num_subtasks', 'permalink_url', 'assignee_status',
        'created_at', 'modified_at', 'start_on', 'due_at', 'due_on', 'completed', 'completed_at',
        'parent', 'assignee', 'assignee.name',
        'followers.name', 'projects.name', 'projects.team.name',
        'workspace.name', 'workspace.is_organization'
    ].join(',')
    ...
    for (const workspace of workspaces) {
        ...
        await db.workspace.upsert({
            ...
            create: {gid: workspaceId, name: fullWorkspace.name, isOrganization: fullWorkspace.is_organization},
            update: {name: fullWorkspace.name, isOrganization: fullWorkspace.is_organization}
        })
        ...
        do {
            ...
            const {data, nextPage, _response: {next_page}} = page
            ...
            for (const task of data) {
                ...
                if (parent && !parentInDb) {
                    ...
                    const parentTask = await client.tasks.findById(parent.gid, {opt_fields})
...
export default {
    ...
    incrementalSync: async (db: PrismaClient) => {
