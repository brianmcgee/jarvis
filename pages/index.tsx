...
const Home = (): ReactElement | null => {
    const { tasksForToday, upcomingTasks, tasksForLater } = useDashboardQuery({
                                    ...
                                    <div className="flex items-center text-sm text-gray-500">
                                        ...
                                        <p>
                                            {dueAt && (
                                                <span>
                                                    Due{' '}
                                                    <time dateTime={dueAt}> {dayjs(dueAt).format('DD-MM-YYYY')}</time>
                                                </span>
                                            )}
                                        </p>
                                    </div>
            ...
            <main className="flex flex-col items-center bg-gray-50 w-full h-screen py-16">
                <div className="container mx-auto bg-gray">
                    ...
                    <div className="bg-white shadow overflow-hidden sm:rounded-md">
                        <ul className="divide-y divide-gray-200">{tasksForToday.map((t) => renderTask(t))}</ul>
                    </div>
                    ...
                    <div className="bg-white shadow overflow-hidden sm:rounded-md">
                        <ul className="divide-y divide-gray-200">{upcomingTasks.map((t) => renderTask(t))}</ul>
                    </div>

                    <div className="pb-5 border-b border-gray-200 mt-8">
                        <h3 className="text-lg leading-6 font-medium text-gray-900">Later</h3>
                    </div>

                    <div className="bg-white shadow overflow-hidden sm:rounded-md">
                        <ul className="divide-y divide-gray-200">{tasksForLater.map((t) => renderTask(t))}</ul>
                    </div>
                </div>
            </main>
        </div>
